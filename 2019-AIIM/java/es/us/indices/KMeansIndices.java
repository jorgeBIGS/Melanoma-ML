...
public class KMeansIndices extends weka.clusterers.SimpleKMeans {
	...
	private Indice calcularBDDunn() {
		...
		try {
			...
			for (Cluster cluster : clusters) {
				for (Cluster cluster2 : clusters) {
					if (!cluster.equals(cluster2) && cluster.getCentroide() != null
							&& cluster2.getCentroide() != null) {
			...
			// get the maximum distance of the points to the centroid of the cluster they
			// belong to
			for (Cluster cluster : clusters) {
	...
	private Indice calcularDavidBouldin() {
		...
		if (numberOfClusters == 1) {
			throw new RuntimeException("Impossible to evaluate Davies-Bouldin index over a single cluster");
		} else {
					...
					if (clusters.get(i).getCentroide() != null) {
						...
						for (int j = 0; j < numberOfClusters; j++)
							...
							if (i != j && clusters.get(j).getCentroide() != null) {
								double val = (withinClusterDistance[i] + withinClusterDistance[j]) / m_DistanceFunction
										.distance(clusters.get(i).getCentroide(), clusters.get(j).getCentroide());
								if (val > max)
									max = val;
							}
					}
